...
@SubConfigClass<TAGS>({tags: {client: true}, softReadonly: true})
export class ClientOidcConfig {
  @ConfigProperty<boolean, ClientConfig>({
    tags: {
      name: $localize`Enable OIDC`,
      priority: ConfigPriority.advanced,
      uiResetNeeded: {server: true},
    },
    description: $localize`Enable Single-Sign-On via OIDC"`,
  })
  enabled: boolean = false;

  @ConfigProperty<boolean, ClientConfig>({
    tags: {
      name: $localize`Automatic OIDC`,
      priority: ConfigPriority.advanced,
      uiResetNeeded: {server: true},
    },
    description: $localize`Instead of showing the log in page, automatically start an OIDC log in. This prevents users from `,
  })
  automatic: boolean = false;

  @ConfigProperty<boolean, ClientConfig>({
    tags: {
      name: $localize`Name of OIDC issuer`,
      priority: ConfigPriority.advanced,
      uiResetNeeded: {server: true},
    },
    description: $localize`Name of the OpenID Connect issuer used in messages to user, e.g. "Microsoft"`,
  })
  issuerName: string;
}
...
export class ClientUserConfig {
  ...
  @ConfigProperty({
    type: ClientOidcConfig, tags: {
      name: $localize`OpenID Connect SSO`,
      priority: ConfigPriority.advanced,
      uiResetNeeded: {server: true},
      relevant: (c: any) => c.authenticationRequired === true
    },
    description: $localize`Configuration for Single-Sign-On via OpenID Connect`,
  })
  Oidc: ClientOidcConfig = new ClientOidcConfig();
}
